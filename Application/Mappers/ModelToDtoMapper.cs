    ...
    public static class ModelToDtoMapper
    {
        ...
        public static List<TicketDto> ToDto(this List<Ticket> tickets)
        {
            foreach (var ticket in tickets)
            {
                ...
                persianParse.GetYear(ticket.Schedule.Date), persianParse.GetMonth(ticket.Schedule.Date), persianParse.GetDayOfMonth(ticket.Schedule.Date));
                DateTime PersianDate = Convert.ToDateTime(Date);
                ticket.Schedule.SetPersianDate(PersianDate);
            }
            ...
            return tickets.Select(x => new TicketDto()
            {
                ...
                CellPhone = x.User.PhoneNumber,
                EFunType = x.FunType,
                ScheduleId = x.Schedule.Id,
                FunId = x.Schedule.FunId,
                TotalPrice = x.Schedule.Price,
                SubmitPersianDate = x.SubmitDate.ToString(),
                ExecutePersianDate = x.Schedule.Date.ToString(),
                UserId = x.User.Id,
